...
- Current Chrome Web Store guidance treats that local handling as user-data handling and requires a prominent in-product disclosure plus an affirmative action before handling begins. The present automatically injected content script has no such first-use flow, so the prepared 0.1.2 package is blocked from submission pending an approved product correction.
