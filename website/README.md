...
The repository is connected to the public `clementmst/search-keyboard-navigator` GitHub repository. The website remains undeployed. Before the first deployment, the user must approve the reviewed merge to `main` and enable GitHub Pages with **GitHub Actions** as the source. After that one-time setup, reviewed pushes to `main` can deploy this folder through `.github/workflows/deploy-pages.yml`.
...
The Chrome Web Store install link uses the current extension ID `eifanigljpfnmmdfeefjdioelbkgmeja`. Update `softwareVersion` and the sitemap date with each public release.
